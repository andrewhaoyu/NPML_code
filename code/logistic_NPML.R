...
cppFunction('NumericMatrix E_step_pp(NumericVector uu,NumericVector b,NumericMatrix x,NumericVector N,NumericVector obs){
            int KK = N.size();
            int p = b.size();
            ...
            pp(i,j) = pow((1/(1+exp(uu(j)+xbeta))),(N(i)-obs(i)))*(exp(uu(j)+xbeta)/(1+exp(uu(j)+xbeta)))^obs(i);
            }
            }
            return(pp);
            }')
...
gr_u <- function(u,N,ww_vec,b,x,K,observation){
  ...
  for(l in 1:observation){
    ...
    xbeta <- x%*%b
    temp_Qu <- 1-N_temp*logit_inver(u+xbeta)
...
#### NPML iteration
NPML_logisitic_function <- function(y,x,obs){
  p <- ncol(x)
  K <- length(y)
  uu <- seq(0,1,by = 1/(K-1))
  p <- rep(1/K,K)
