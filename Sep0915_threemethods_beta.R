...
ParaBetaEstimateFunction <- function(N){
  fit <-  mle2(BetaGeometricLikehood2,method="SANN",start = list(Logit.ThetaBar.Par=logit(True.Theta.Mean),Log.M.Par=M))
  a<-coef(fit)[[1]]
...
Theta.Bar.Vector <- c(0.1,0.2,0.3,0.5)
M.Vector <- c(0.05,0.25,0.5,1,1.5,2,5,10,25,10000)
K.Vector <- c(10,20,50,75,100,150,200)
Theta.Low <- 0.005
Theta.High <- 0.995
for(i1 in 1:length(Theta.Bar.Vector)) {
  for(i2 in 1:length(M.Vector)) {
    for(i3 in 1:length(K.Vector)) {
      ...
      Para.Beta.Estimate.Result1 <- NULL
      Para.Beta.Estimate.Result2 <- NULL
...
save.image("/home/student/hzhang1/R/project_Tom/Sep0915_comparethreemethods_beta_result_theta0.1.Rdata")
...
NPMLEstimateFunction <- function(param, data) {
  #beginning setting of NPML:
  # NN is the geometric sampling result
  # UU is the probability mass point of NPML
  # UU begins with 1:K/(K+1)
  ...
  KK <- length(NN)
  UU <- param[1:KK]
  pp <- param[(KK+1):length(param)]
  Y <- rep(1,KK)
  ww <- matrix(nrow=KK,ncol=KK) 
  for(k in 1:KK) {
    ww[k,] <- mapply(function(x,y) recursion(x,y,NN[k],Y[k]),pp,UU)
    ww[k,] <- ww[k,]/sum(ww[k,])
  }  #k to K
...
NPMLEloglik <- function(param, data){
  UU <- param[1:KK]
  pp <- param[(KK+1):length(param)]
  loglik <- log(sum(pp * dgeom(data, prob = UU)))
  return(-loglik)
}
...
system.time(f1 <- fpiter(par = param.start, objfn = NPMLEloglik, fixptfn = NPMLEstimateFunction, data = N))

system.time(f2 <- squarem(par = param.start, objfn = NPMLEloglik, fixptfn = NPMLEstimateFunction, data = N))
